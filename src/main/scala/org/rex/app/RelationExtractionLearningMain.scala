  ...
  parser.parse(args, RelConfig.emptyUnsafe) match {
    ...
    case Some(config) =>
      if (!(config.lr.isDefined || config.ev.isDefined || config.ex.isDefined)) {
        println(
          "ERROR: One of LearningCmd, EvaluationCmd, or ExtractionCmd must be defined.\n")
        parser.showUsage
        System.exit(1)
      }
      ...
      if (config.cmd == LearningCmd && config.lr.get.modelOut.isEmpty) {
        println(
          "ERROR: Command is \"learning\" and no model output path is specified.\n")
        parser.showUsage
        System.exit(1)
      }
      ...
      config.ex.foreach {
        case Extraction(rawInput, reader, modelIn, extractOut) =>
          throw new RuntimeException(
            "ERROR: Extraction command unimplemented.")
      }
